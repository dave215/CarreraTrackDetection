...
def bild_umwandeln(mask):
    img_debug = cv.cvtColor(mask, cv.COLOR_GRAY2BGR)
...
for i in range(0, len(upper_values)):
    # Maske in Funktion erstellen
    temp = maske_erstellen(frame, lower_value, upper_values[i], area_x, area_y)

    mask_genau.append(temp)


# Masken anzeigbar machen
mask1 = bild_umwandeln(mask[0])
mask2 = bild_umwandeln(mask[1])
mask3 = bild_umwandeln(mask[2])
mask4 = bild_umwandeln(mask[3])
mask5 = bild_umwandeln(mask[4])
mask6 = bild_umwandeln(mask[5])
